    ...
    if (loading)
        return <p className="text-white p-6">Loading approved reservations...</p>;
    if (!user) return null;

    return (
        <div className="min-h-screen bg-black text-white p-6">
        <h1 className="text-3xl font-bold mb-6">Approved Reservations</h1>
        {reservations.length === 0 ? (
            <p className="text-gray-400">Belum ada reservasi yang disetujui.</p>
        ) : (
            ...
            {reservations.map((res) => (
                ...
                <p className="text-green-400 font-semibold mt-2">✅ Disetujui</p>
                </div>
            ))}
            </div>
        )}
